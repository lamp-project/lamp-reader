<template>
  <ion-card>
    ...
    <ion-card-header class="ion-inherit-color md hydrated">
      <img class="header-img" :src="book.cover" />
    </ion-card-header>
    ...
    <ion-card-content>
      ...
      <ion-row>
        ...
        <ion-col>
          It has
          <h2>{{ book.uniqueWordsCount }}</h2>
          <h3>Unique words.</h3>
        </ion-col>
      </ion-row>
      <hr />
      <ReadMore :value="meta.description || ''" :size="35" />
      <hr />
...
export default defineComponent({
  async setup() {
    ...
    const book = bookRepository.findByIdOrThrow(id);
    const meta = await book.getMeta();
    // 3- loading book repost
    const words = await book.getWords();
    // 4- loading library
    const exists = ref(!!(await libraryStore.get(id)));
    // 5- load userWords
    await userWordStore.initialise();
    return { book, meta, exists, words };
  },
  components: {
    IonCard,
    IonCardHeader,
  ...
  computed: {
    readability() {
      let knownWords = 0;
      const userWords = this.userWords.map(({ word }) => word);
      for (const word in this.words) {
        if (Object.prototype.hasOwnProperty.call(this.words, word)) {
      ...
      return Math.round((knownWords * 100) / this.book.wordsCount);
    },
  },
  methods: {
